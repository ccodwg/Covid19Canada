...
def wat_cases():
    return workbook_wat.getWorksheet('Latest: total cases').data.iloc[0, 2]
def wat_mortality():
    return workbook_wat.getWorksheet('Latest: Deaths').data.iloc[0, 2]
def wat_recovered():
    return workbook_wat.getWorksheet('Latest: Resolved cases').data.iloc[0, 2]
